...
const AUTH_KEY = 'tuc_auth_clipai';
const USER_KEY = 'clipai_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie
      .split('; ')
      .find(row => row.startsWith('clipai_user='))
      ?.split('=')[1];
    ...
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'clipai_user=; max-age=0; path=/clipai/';
      } catch (e) {
        console.error(e);
      }
    }
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri =
      ...
      `${window.location.origin}/clipai/callback`;
