# CREATION.md — Fees Comparison Dashboard (Ghana University Fees Dashboard)
...
- **Undergraduate** — freshman vs continuing fees per institution.
- **International** — converted (USD × 15 cedi) fees for African and non-African applicants.
- **Postgraduate** — Master/MPhil/MBA/PhD/MBA programmes.

Beyond the public dashboard, a **password-gated Admin Panel** lets authorised staff (a) edit fees in-place and (b) view the audit log, security/credential settings, and a system-health placeholder. All data lives in React `useState` providers — there is **no backend API** in the current build (a `backend/` workspace placeholder exists for future API work).
...
| Container | node:24-alpine → nginx:alpine (or `serve` on 4173) | — |
...
├── index.html                  # EduData Ghana brand, fonts
├── index.tsx                   # createRoot → <App />
├── index.css                   # Tailwind directives
├── App.tsx                     # ThemeProvider → AuthProvider → DataProvider → MainContent (Header + view switch)
├── types.ts                    # Verbatim type contract — see §6
├── package.json                # name: ghana-university-fees-dashboard
├── pnpm-lock.yaml
...
├── Dockerfile                  # node:24-alpine builder; runtime serve dist on 4173
├── Dockerfile.prod
├── nginx.conf                  # SPA fallback try_files $uri /index.html
├── .env.local                  # GEMINI_API_KEY (legacy AI Studio export)
├── .npmrc
├── .dockerignore
├── DEPLOYMENT.md
├── README.md
├── SRS.md                      # IEEE SRS v1.1
├── metadata.json
├── public/                     # static assets
├── docs/                       # architecture diagrams, deployment guide
├── migrations/                 # placeholder for backend SQL migrations
├── backend/                    # workspace placeholder (no live API yet)
├── components/
│   ├── FeesComparisonDashboard.tsx   # Public tab + chart + insights
│   └── AdminPanel.tsx                # Login, Logs, Data Mgmt, Security, Health tabs
├── contexts/
│   ├── ThemeContext.tsx        # light | dark | high-contrast → body class
│   ├── AuthContext.tsx         # in-memory password + audit log
│   └── DataContext.tsx         # seeded fees data + updateFee()
├── src/                        # legacy React Scripts artefacts (kept for fallback)
│   ├── AuthGate.jsx
│   ├── components/, contexts/, services/, pages/
│   ├── __tests__/
│   └── index.js
└── WEB-INF/                    # Tomcat context for WAR deploy
```

The **canonical entry tree** is `index.tsx → App.tsx → components/`. The `src/` folder is a legacy CRA tree retained only for the WAR/Tomcat fallback path.
...
│  ─────────────────────────────────────────────────────    │
│  [📊 EduData Ghana]            [Light|Dark|HC] [Admin▸]  │
├────────────────────────────────────────────────────────────┤
│  MAIN (max-w-7xl, py-8 sm:py-12)                          │
│  ┌──────────────────────────────────────────────────────┐ │
│  │  TABS: Undergraduate | International | Postgraduate  │ │
│  │  ──────────────────────────────────────────────────  │ │
│  │  RECHARTS BAR CHART (ResponsiveContainer)            │ │
│  │  X axis: institution names                           │ │
│  │  Y axis: GH₵ values (formatted "GH₵Nk")              │ │
│  │  Bars (UG): freshman + continuing                    │ │
│  │  Bars (Intl/PG): single fee                          │ │
│  │  Tooltip: name + value (GH₵...) + type badge         │ │
│  │  ──────────────────────────────────────────────────  │ │
│  │  KEY OBSERVATIONS panel — text insights              │ │
│  └──────────────────────────────────────────────────────┘ │
├────────────────────────────────────────────────────────────┤
│  FOOTER — © year EduData Ghana — Privacy / Terms / Contact│
└────────────────────────────────────────────────────────────┘
```
...
## 7. Seeded Data (`contexts/DataContext.tsx` — required)

### 7.1 Undergraduate (19 rows — fees in GH₵)
...
### 7.2 International (9 rows — base $ × 15 cedi multiplier)
Public: UG (Humanities) African 4328×15, UG (Humanities) Non-African 5109×15, UG (Admin/Law) 5336×15, KNUST (Humanities) 4354×15, KNUST (Business/Law) 6054×15, KNUST (Engineering) 6204×15, KNUST (Medicine) 7487×15.
Private: Lancaster (Foundation) 7000×15, Lancaster (Undergraduate) 9000×15.

### 7.3 Postgraduate (10 rows)
Public: UG (MA – One Year) 13074, UG (MPhil/MBA) 13551, UG (Law – ADR/HRA) 22485, UG (EPM – Regular) 18540, UG (EPM – Weekend) 25785, UG (PhD – Humanities) 10256, UG (PhD – Admin) 11694.
Private: Academic City (Data Sci) 47250/2, Academic City (Cyber Sec) 47250/2, Lancaster (MBA) (12000×15)/2.
...
- In-memory password (default `admin123`) — **no localStorage persistence; session loss on reload is intentional** for the public preview build.
- `login(password) → boolean`. Calls `logAction('LOGIN', 'Admin logged in successfully')` on success or `logAction('LOGIN_FAILED', 'Invalid password attempt')` on failure.
- `logout()` → `logAction('LOGOUT', 'Admin logged out')`.
- `updatePassword(newPassword)` → `logAction('SECURITY_UPDATE', 'Admin password changed')`.
...
The chart and admin shell consume `useTheme()` and switch palettes inline — see §11.
...
- Currency helpers:
  ```ts
  formatCurrency(v)     → `GH₵${v.toLocaleString()}`
  formatAxisCurrency(v) → v >= 1000 ? `GH₵${(v/1000).toFixed(0)}k` : `GH₵${v}`
  ```
- Tooltip shows: institution name, fee value via `formatCurrency`, public/private type badge.
- A "Key Observations" insights panel below the chart summarises the active view in 3–5 bullets.
...
- On submit calls `login(passwordInput)`; on false → "Incorrect password. Please try again."

### 13.2 Authenticated view
Sticky header: avatar circle "A", "Administrator", "● System Active" status, **Sign Out** button (red).
...
| `settings` | Security | New-password input + "Update Credentials" button → `updatePassword()` |
| `health` | System Health | Placeholder card; "Launch Diagnostics (Preview)" button — Phase 3 hook for the test runner |
...
Dev server port **3000**, host `0.0.0.0`. `base: './'` for Tomcat-relative deployment. Plugins: `react()`, `tailwindcss()`. Vite `define` shims `process.env.API_KEY` and `process.env.GEMINI_API_KEY` from the `GEMINI_API_KEY` env var (legacy AI Studio scaffolding — kept for backward compatibility).
...
1. `FROM node:24-alpine AS builder` — `corepack enable`, `pnpm install --frozen-lockfile || npm install`, copy source, `pnpm run build`.
2. `FROM node:24-alpine` — install global `serve`, copy `dist/`, `EXPOSE 4173`, `HEALTHCHECK` hits `http://localhost:4173/health`, `CMD ["serve", "-s", "dist", "-l", "4173"]`.
...
Coverage target: ≥70% across branches/functions/lines/statements (TUC monorepo standard).
...
- Header `Admin Access` / `Public Dashboard` toggle button — must remain keyboard-accessible (`<button>`).
...
| AC-6 | International and postgraduate views render single fee bars; values formatted `GH₵Nk` on the Y axis |
...
| AC-15 | All chart values are comma-separated for thousands (e.g. `GH₵41,580`) |
