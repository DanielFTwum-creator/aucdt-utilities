...
const AUTH_KEY = 'tuc_auth_aucdt-msee-aptitude-test';
const USER_KEY = 'aucdt-msee-aptitude-test_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie.split('; ').find(row => row.startsWith('aucdt-msee-aptitude-test_user='))?.split('=')[1];
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'aucdt-msee-aptitude-test_user=; max-age=0; path=/aucdt-msee-aptitude-test/';
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/aucdt-msee-aptitude-test/callback`;
