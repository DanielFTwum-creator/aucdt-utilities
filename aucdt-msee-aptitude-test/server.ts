...
// Attach the decoded JWT to the request in authenticateToken.
declare global {
...
// --- Database Connection ---
// Fleet DB pattern (lems / tuc-rms / dmcdai): the app MariaDB is the 10.3 instance
// on port 3306; the 3307 instance is the LMS — do not use it. Each app has its own
// database and a scoped, non-root user; credentials live in the app .env on the
// server (localhost-only grants, never committed). DB_* are set once at
// provisioning time — the deploy preserves them.
const dbConfig = {
...
app.post('/api/auth/login', async (req: Request, res: Response) => {
    ...
    if (!email || !password) return res.status(400).json({ error: 'Email and password are required.' });
    try {
        const [rows] = await pool.execute<RowDataPacket[]>('SELECT id, email, password_hash, role FROM users WHERE email = ?', [email]);
        ...
        const accessToken = jwt.sign({ id: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET as string, { expiresIn: '8h' });
...
app.get('/api/exams', async (req: Request, res: Response) => {
    try {
        const [rows] = await pool.execute<RowDataPacket[]>('SELECT id, name, description, subject, questions FROM exams ORDER BY created_at DESC');
...
// PROGRESS
app.get('/api/progress/:examId', authenticateToken, async (req: Request, res: Response) => {
...
app.post('/api/progress/:examId', authenticateToken, async (req: Request, res: Response) => {
    const { examId } = req.params;
    const userId = req.user!.id;
...
app.delete('/api/progress/:examId', authenticateToken, async (req: Request, res: Response) => {
    const { examId } = req.params;
    const userId = req.user!.id;
    try {
        await pool.execute(
...
// GEMINI AI
app.post('/api/generate', authenticateToken, isAdmin, async (req: Request, res: Response) => {
...
app.get(['/api/health', '/aucdt-msee-aptitude-test/api/health'], (_req: Request, res: Response) => res.json({ ok: true }));

// --- Google OAuth callback (single sign-on) ---
// Google redirects here with ?code=...; relay the exchange through WMS (no secret
// held), map the verified email to a users row, mint the exam JWT, and set the
// cookies the frontend reads. Registered before the static/catch-all so it wins.
app.get(['/callback', '/aucdt-msee-aptitude-test/callback'], async (req: Request, res: Response) => {
  const { code, error } = req.query;
  if (error) return res.redirect(`/aucdt-msee-aptitude-test/?error=${encodeURIComponent(String(error))}`);
  ...
  try {
    ...
    let userRow: { id: number; role: string };
    const [existing] = await pool.execute<RowDataPacket[]>('SELECT id, role FROM users WHERE email = ?', [email]);
    if (existing.length > 0) {
      userRow = existing[0] as { id: number; role: string };
    } else {
      try {
        const [result] = await pool.execute<ResultSetHeader>('INSERT INTO users (email, password_hash) VALUES (?, ?)', [email, 'google-oauth-no-password']);
        userRow = { id: result.insertId, role: 'student' };
      } catch (e: any) {
        if (e && e.code === 'ER_DUP_ENTRY') {
          const [again] = await pool.execute<RowDataPacket[]>('SELECT id, role FROM users WHERE email = ?', [email]);
          userRow = again[0] as { id: number; role: string };
        } else { throw e; }
      }
    }
    const accessToken = jwt.sign({ id: userRow.id, email, role: userRow.role }, process.env.JWT_SECRET as string, { expiresIn: '8h' });
