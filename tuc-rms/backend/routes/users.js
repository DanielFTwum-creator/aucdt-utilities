...
router.get('/', auth, requireRole('registrar', 'qa_officer'), async (req, res) => {
  try {
    const [rows] = await db.execute(
      'SELECT id, full_name, email, role, department, staff_id, is_active, created_at FROM tuc_rms_users ORDER BY role, full_name'
    );
    res.json(rows);
  } catch (err) {
...
router.get('/lecturers', auth, async (req, res) => {
  try {
    const [rows] = await db.execute(
      "SELECT id, full_name, email, department, staff_id FROM tuc_rms_users WHERE role = 'lecturer' AND is_active = 1 ORDER BY full_name"
    );
    res.json(rows);
  } catch (err) {
    res.status(500).json({ message: 'Server error' });
...
router.post('/', auth, requireRole('registrar'), async (req, res) => {
  try {
    ...
    const [result] = await db.execute(
      'INSERT INTO tuc_rms_users (full_name, email, password_hash, role, department, staff_id) VALUES (?,?,?,?,?,?)',
    ...
    await db.execute('INSERT INTO audit_log (user_id, action, table_name, record_id, details) VALUES (?,?,?,?,?)',
      [req.user.id, 'CREATE_USER', 'tuc_rms_users', result.insertId, `Created user: ${email}`]);
...
router.put('/:id', auth, requireRole('registrar'), async (req, res) => {
  try {
    ...
    await db.execute(
      'UPDATE tuc_rms_users SET full_name=?, email=?, role=?, department=?, staff_id=?, is_active=? WHERE id=?',
...
router.post('/:id/reset-password', auth, requireRole('registrar'), async (req, res) => {
  try {
    ...
    await db.execute('UPDATE tuc_rms_users SET password_hash=? WHERE id=?', [hash, req.params.id]);
