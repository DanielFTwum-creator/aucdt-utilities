# midjourney-prompt-helper — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/midjourney-prompt-helper/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/midjourney-prompt-helper/",
    [switch]$Build = $false
)
...
$SUBFOLDER     = "midjourney-prompt-helper"
...
Log "INFO" "========================================" Cyan
Log "INFO" "midjourney-prompt-helper DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/midjourney-prompt-helper_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning midjourney-prompt-helper (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set midjourney-prompt-helper
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /midjourney-prompt-helper/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /midjourney-prompt-helper/index.html [QSA,L]
</IfModule>
...
$timeStr = if ($elapsed -ge 60) { "$([math]::Floor($elapsed/60))m $([math]::Round($elapsed % 60))s" } else { "${elapsed}s" }
Log "SUCCESS" "DEPLOYMENT COMPLETE in $timeStr — $RemotePath" Green
