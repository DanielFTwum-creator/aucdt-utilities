...
/**
 * Mandatory TOTP MFA for TSAPro users (per-user mfaRequired). Shown when the OAuth
 * callback returns an mfa_ticket. Two modes on one page:
 *   - Verify (returning, enrolled): enter 6-digit code → JWT.
...
const MAROON = '#7a1722';
const card: React.CSSProperties = { background: '#fff', borderRadius: 16, padding: '36px 32px', width: '100%', maxWidth: 380, border: '1px solid #e5e7eb', boxShadow: '0 8px 40px rgba(0,0,0,0.08)', fontFamily: 'Inter, system-ui, sans-serif' };
const h1: React.CSSProperties = { fontSize: 18, fontWeight: 800, color: MAROON, marginTop: 0 };
const sub: React.CSSProperties = { fontSize: 13, color: '#64748b' };
const codeInput: React.CSSProperties = { width: '100%', padding: '12px', fontSize: 22, letterSpacing: 8, textAlign: 'center', border: '1px solid #d1d5db', borderRadius: 8, marginBottom: 14, boxSizing: 'border-box' };
const errText: React.CSSProperties = { color: '#c0392b', fontSize: 13, marginBottom: 12 };
const btn: React.CSSProperties = { width: '100%', padding: '12px', background: MAROON, color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: 'pointer' };
const linkBtn: React.CSSProperties = { width: '100%', padding: '10px', background: 'none', border: 'none', color: '#64748b', fontSize: 12, cursor: 'pointer', marginTop: 8 };
