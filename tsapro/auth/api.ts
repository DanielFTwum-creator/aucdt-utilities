...
//
// Unlike the WMS app (same-origin), TSAPro is a separate host, so every call is
// prefixed with the WMS auth host. Override via VITE_AUTH_BASE for local dev.

const AUTH_BASE = (import.meta as any).env?.VITE_AUTH_BASE ?? 'https://wms.techbridge.edu.gh';
const url = (path: string) => (path.startsWith('http') ? path : AUTH_BASE + path);
