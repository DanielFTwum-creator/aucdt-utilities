# luxthumb-agent — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/luxthumb-agent/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/luxthumb-agent/",
    [switch]$Build = $false
)
...
Log "INFO" "========================================" Cyan
Log "INFO" "luxthumb-agent DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/luxthumb-agent_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning luxthumb-agent (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set luxthumb-agent
cd luxthumb-agent
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /luxthumb-agent/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /luxthumb-agent/index.html [QSA,L]
</IfModule>
