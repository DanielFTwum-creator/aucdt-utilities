# poster — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/poster/
...
if ($Build) {
    ...
    $buildDir = "/tmp/poster_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning poster (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set poster
cd poster
