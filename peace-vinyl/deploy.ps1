# peace-vinyl — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/peace-vinyl/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/peace-vinyl/",
    [switch]$Build = $false
)
...
if ($Build) {
    ...
    $buildDir = "/tmp/peace-vinyl_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning peace-vinyl (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set peace-vinyl
cd peace-vinyl
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /peace-vinyl/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /peace-vinyl/index.html [QSA,L]
</IfModule>
...
ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -o ServerAliveCountMax=3 $RemoteHost "ss -tlnp | grep -q ':3001' && echo 'OK port 3001 listening' || echo 'WARN port 3001 not found'"
...
Log "SUCCESS" "URL  : https://ai-tools.techbridge.edu.gh/peace-vinyl/" Green
