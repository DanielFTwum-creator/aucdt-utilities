...
$SUBFOLDER     = "dmcdai-digital-media-communication-design"
...
Log "INFO" "========================================" Cyan
Log "INFO" "dmcdai-digital-media-communication-design DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    scp -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -o ServerAliveCountMax=3 ".env.local" "${RemoteHost}:/tmp/.env.dmcdai" 2>$null | Out-Null
    $serverScript = @"
set -e
...
log '[2/5] Cloning dmcdai-digital-media-communication-design (sparse, depth 1)...'
...
ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -o ServerAliveCountMax=3 $RemoteHost "cd $RemotePath && pnpm install --prod --silent"
...
ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -o ServerAliveCountMax=3 $RemoteHost "ss -tlnp | grep -q ':3014' && echo 'OK port 3014 listening' || echo 'WARN port 3014 not found'"
