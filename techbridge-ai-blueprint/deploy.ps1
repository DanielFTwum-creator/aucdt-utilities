...
# Deploys frontend (dist/) + backend (server.ts on port 3005) with OAuth.
...
bash -c "cd 'C:\Development\github\aucdt-utilities\techbridge-ai-blueprint' && scp -r -o StrictHostKeyChecking=no dist/* $RemoteHost`:$RemotePath 2>&1 | head -20"
...
<IfModule mod_rewrite.c>
  ...
  RewriteRule ^callback http://localhost:3005/callback [P,L]
...
ssh -o StrictHostKeyChecking=no $RemoteHost "fuser -k 3005/tcp 2>/dev/null ; sleep 1 ; cd $RemotePath && NODE_ENV=production setsid nohup tsx server.ts > server.log 2>&1 < /dev/null &" 2>&1 | Out-Null
...
ssh -o StrictHostKeyChecking=no $RemoteHost "ss -tlnp 2>/dev/null | grep -q ':3005' && echo 'OK Port 3005 listening' || echo 'X Port 3005 NOT listening'"
ssh -o StrictHostKeyChecking=no $RemoteHost "curl -sS -o /dev/null -w 'HTTP %{http_code}' http://localhost:3005/api/health && echo ' (backend health OK)'"

Write-Host ""
Write-Host "Deployment complete!" -ForegroundColor Green
Write-Host "URL: https://ai-tools.techbridge.edu.gh/blueprint/"
Write-Host "Backend port: 3005`n"
