...
$SUBFOLDER     = "techbridge-ai-blueprint"
...
Log "INFO" "========================================" Cyan
Log "INFO" "techbridge-ai-blueprint DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/tb-ai-blueprint_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning techbridge-ai-blueprint (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set techbridge-ai-blueprint
...
if command -v pm2 &>/dev/null; then
  if pm2 describe tb-ai-blueprint &>/dev/null; then
    ...
    cd $RemotePath && PORT=3005 pm2 start server.ts --name tb-ai-blueprint --interpreter npx --interpreter-args tsx
...
ssh -o StrictHostKeyChecking=no $RemoteHost "ss -tlnp | grep -q ':3005' && echo 'OK port 3005 listening' || echo 'WARN port 3005 not found'"
