...
#
# Server-side build (fleet standard): only a tiny build script is uploaded over
# your link; the SERVER clones main from GitHub and runs pnpm build there, then
# rsyncs dist/. No local node_modules needed, fast on a slow link. Builds from
# whatever is on main.
...
Log "INFO" "========================================" Cyan
Log "INFO" "bionicskins DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/bionicskins_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning bionicskins (sparse, depth 1)...'
