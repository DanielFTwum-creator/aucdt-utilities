...
const AUTH_KEY = 'tuc_auth_magic_reader';
const USER_KEY = 'magic_reader_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie.split('; ').find(row => row.startsWith('magic_reader_user='))?.split('=')[1];
    if (cookieValue) { try { const userData = JSON.parse(atob(decodeURIComponent(cookieValue))) as User; localStorage.setItem(USER_KEY, JSON.stringify(userData)); sessionStorage.setItem(AUTH_KEY, '1'); setUser(userData); setAuthed(true); document.cookie = 'magic_reader_user=; max-age=0; path=/magic-reader/'; } catch (e) { console.error('Failed to parse user cookie:', e); } }
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/magic-reader/callback`;
