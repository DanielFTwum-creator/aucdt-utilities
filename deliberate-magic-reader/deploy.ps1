# deliberate-magic-reader — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/magic-reader/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/magic-reader/",
    [switch]$Build = $false
)
...
$SUBFOLDER     = "deliberate-magic-reader"
...
Log "INFO" "========================================" Cyan
Log "INFO" "deliberate-magic-reader DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/deliberate-magic-reader_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning deliberate-magic-reader (sparse, depth 1)...'
...
ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -o ServerAliveCountMax=3 $RemoteHost "ss -tlnp | grep -q ':3008' && echo 'OK port 3008 listening' || echo 'WARN port 3008 not found'"
...
Log "SUCCESS" "URL  : https://ai-tools.techbridge.edu.gh/magic-reader/" Green
