...
if ($envContent -notmatch "GEMINI_API_KEY") {
    Write-Host "Error: GEMINI_API_KEY missing in .env.local" -ForegroundColor Red
    exit 1
}
...
ssh -o StrictHostKeyChecking=no $RemoteHost "fuser -k 3006/tcp 2>/dev/null ; sleep 1 ; cd $RemotePath && NODE_ENV=production setsid nohup tsx server.ts > server.log 2>&1 < /dev/null &" 2>&1 | Out-Null
...
Write-Host "URL: https://ai-tools.techbridge.edu.gh/glucose"
Write-Host "Backend port: 3006`n"
