...
# Inject it from WMS custody (/opt/tuc-wms/.env), BOM/CR/null-stripped (Pattern 21), and ensure
# NODE_ENV=production so the static SPA is served. The key never touches the dev machine.
...
if command -v pm2 &>/dev/null; then
  ...
  pm2 delete deep-dub-vibes-player &>/dev/null || true
  cd $RemotePath && NODE_ENV=production PORT=3023 pm2 start server.ts --name deep-dub-vibes-player --interpreter npx --interpreter-args tsx --cwd $RemotePath >/dev/null
  echo 'pm2: hard restart deep-dub-vibes-player (Pattern 23)'
  pm2 save --force &>/dev/null
fi
"@
$b64r = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($restartCmd.Replace("`r", "")))
# Actually RUN the restart (this line was missing — the deploy built $b64r but never
# executed it, so the pm2 process was never restarted and stayed on the old server.js).
