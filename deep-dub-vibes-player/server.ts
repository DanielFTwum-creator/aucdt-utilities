...
// (verified against the live nginx location), and the SPA is built with an absolute
// base of /deep-dub-vibes-player/. Strip the prefix so the routes below (/callback,
...
if (!GEMINI_PROXY_KEY) {
  console.warn('[deep-dub-vibes-player] WARNING: GEMINI_PROXY_KEY not set — Google login will return 503');
}
...
app.get('/callback', (req: Request, res: Response) => {
  ...
  if (error) return res.redirect(`/deep-dub-vibes-player/?error=${encodeURIComponent(String(error))}`);
...
app.post('/api/auth/google/token', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      console.error(`[deep-dub-vibes-player] Token exchange failed ${tokenResponse.status}: ${err.slice(0, 300)}`);
    ...
    console.error('[deep-dub-vibes-player] OAuth exchange error:', error);
...
app.listen(Number(PORT), '127.0.0.1', () => {
  console.log(`[deep-dub-vibes-player] OAuth server (WMS relay) listening on http://127.0.0.1:${PORT}`);
});
