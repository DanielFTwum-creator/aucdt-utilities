...
Log "INFO" "=== ANALYTICS DASHBOARD DEPLOYMENT ===" Cyan
...
if (-not (Test-Path "build")) {
    Log "ERROR" "Error: build/ not found. Run with -Build flag." Red
    exit 1
}
...
Get-ChildItem -Path "dist" | ForEach-Object {
    if (# TUC Analytics Dashboard Deployment Script
# Simple SCP-based deployment
...
Log "INFO" "URL: https://ai-tools.techbridge.edu.gh/analytics`n"
# TUC Analytics Dashboard Deployment Script
# Simple SCP-based deployment
...
Log "SUCCESS" "✅ Deployment complete!" Green
Log "INFO" "URL: https://ai-tools.techbridge.edu.gh/analytics`n".PSIsContainer) { scp -r -o StrictHostKeyChecking=no "$(# TUC Analytics Dashboard Deployment Script
# Simple SCP-based deployment
...
Log "SUCCESS" "✅ Deployment complete!" Green
Log "INFO" "URL: https://ai-tools.techbridge.edu.gh/analytics`n".FullName)" "${RemoteHost}:${RemotePath}" }
    else { scp -o StrictHostKeyChecking=no "$(# TUC Analytics Dashboard Deployment Script
# Simple SCP-based deployment
...
Log "SUCCESS" "✅ Deployment complete!" Green
Log "INFO" "URL: https://ai-tools.techbridge.edu.gh/analytics`n"_deployStart = Get-Date
function Log {
    param([string]$Level = "INFO", [string]$Msg, [ConsoleColor]$Color = "White")
    $ts = (Get-Date).ToString("yyyy-MM-dd HH:mm:ss")
    Write-Host "[$ts][$Level] $Msg" -ForegroundColor $Color
