# analytics-refactor — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/analytics-refactor/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/analytics-refactor/",
    [switch]$Build = $false
)
...
Log "INFO" "========================================" Cyan
Log "INFO" "analytics-refactor DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/analytics-refactor_deploy_$commit"
    $serverScript = @"
set -e
...
cd $buildDir
git sparse-checkout set analytics-refactor
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /analytics-refactor/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /analytics-refactor/index.html [QSA,L]
</IfModule>
