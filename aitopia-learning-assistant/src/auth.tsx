...
// WMS SSO gate for aitopia (TUC-ICT-SRS-2026-013, archetype B). aitopia is a
// TUC learning assistant; the WMS gate admits all @techbridge.edu.gh members
// (students + staff). Wraps the app: silently adopts an existing WMS session,
// otherwise shows "Continue with Google". Holds the WMS access token in memory
// and injects it into every same-origin /aitopia/api/ request via a global
// fetch wrapper, so the app components authenticate transparently.

const WMS = (import.meta as any).env?.VITE_WMS_BASE ?? 'https://wms.techbridge.edu.gh';
const APP_ID = 'aitopia';
...
function Login({ reason }: { reason: string }) {
  return (
    <div style={bg}><div style={card}>
      <div style={{ color: accent, fontSize: 22, fontWeight: 800, letterSpacing: 4 }}>AITOPIA</div>
      <div style={{ color: '#64748b', fontSize: 11, letterSpacing: 3, marginTop: 4 }}>LEARNING ASSISTANT</div>
      ...
      <div style={{ color: '#475569', fontSize: 10, marginTop: 14 }}>Techbridge staff &amp; students</div>
    </div></div>
  );
}
