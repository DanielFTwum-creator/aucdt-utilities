# ============================================================
# Lecturer AI Handbook — Deploy Script
# Remote : root@techbridge.edu.gh
# Path   : /var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/lecturer
# Port   : 3042  |  PM2 app: lecturer-ai-handbook
...
$REMOTE      = 'root@techbridge.edu.gh'
$DEPLOY_PATH = '/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/lecturer'
$PORT        = 3042
$PM2_APP     = 'lecturer-ai-handbook'
$HEALTH_URL  = 'https://ai-tools.techbridge.edu.gh/lecturer'
$GITHUB_REPO = 'git@github.com:DanielFTwum-creator/aucdt-utilities.git'
$SUBFOLDER   = 'lecturer-ai-handbook'
...
Log -Level 'INFO' -Msg 'LECTURER AI HANDBOOK DEPLOYMENT'          -Color Cyan
Log -Level 'INFO' -Msg '========================================' -Color Cyan
...
# No build-time secrets: auth is WMS SSO (WMS owns the Google secret) and
# GEMINI_PROXY_KEY is injected server-side. The only build-time var is
# VITE_WMS_BASE, which defaults; .env.local is optional (off-prod override).
...
$envInject = & $SSH @SSH_OPTS $REMOTE "touch ${DEPLOY_PATH}/.env; sed -i '/^GEMINI_API_KEY=/d;/^VITE_GEMINI_API_KEY=/d;/^GEMINI_PROXY_KEY=/d' ${DEPLOY_PATH}/.env; K=`$(grep '^GEMINI_PROXY_KEY=' /opt/tuc-wms/.env | head -1 | cut -d= -f2- | tr -d '\r\000' | LC_ALL=C sed 's/\xef\xbb\xbf//g'); if [ -n `"`$K`" ]; then printf 'GEMINI_PROXY_KEY=%s\n' `"`$K`" >> ${DEPLOY_PATH}/.env; echo 'env: GEMINI_PROXY_KEY injected'; else echo 'WARN: GEMINI_PROXY_KEY not found in WMS'; fi; grep -q '^NODE_ENV=' ${DEPLOY_PATH}/.env || echo 'NODE_ENV=production' >> ${DEPLOY_PATH}/.env; chmod 600 ${DEPLOY_PATH}/.env"
Write-Host $envInject -ForegroundColor DarkGray
if ($envInject -match 'WARN') { Log -Level 'ERROR' -Msg 'GEMINI_PROXY_KEY unavailable — AI would run unconfigured. Aborting.' -Color Red; exit 1 }

Log -Level 'INFO' -Msg 'Step 5: Restarting backend (Pattern 23 hard restart)...' -Color Yellow
# Redirect pm2 start's output to /dev/null — it dumps the entire fleet process
# table (box-drawing chars that mangle over SSH/PowerShell). The echo + the health
# checks below are the real success signal.
$pm2Result = & $SSH @SSH_OPTS $REMOTE "pm2 delete ${PM2_APP} >/dev/null 2>&1 || true; cd ${DEPLOY_PATH}; NODE_ENV=production PORT=${PORT} pm2 start server.ts --name ${PM2_APP} --interpreter npx --interpreter-args tsx --cwd ${DEPLOY_PATH} >/dev/null 2>&1; pm2 save --force >/dev/null 2>&1 || true; echo 'pm2: hard restart (Pattern 23)'"
