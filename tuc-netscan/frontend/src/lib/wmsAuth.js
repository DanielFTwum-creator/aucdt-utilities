...
const WMS = import.meta.env?.VITE_WMS_BASE ?? 'https://wms.techbridge.edu.gh';
...
export async function wmsMfa(path, body) {
...
export async function netscanSsoExchange(wmsToken) {
    ...
    if (!res.ok)
        throw new Error('NetScan SSO exchange failed');
    return res.json();
}
