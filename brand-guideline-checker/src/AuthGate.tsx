...
const AUTH_KEY = 'tuc_auth_brand-guideline-checker';
const USER_KEY = 'brand-guideline-checker_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie.split('; ').find(row => row.startsWith('brand-guideline-checker_user='))?.split('=')[1];
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'brand-guideline-checker_user=; max-age=0; path=/brand-guideline-checker/';
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/brand-guideline-checker/callback`;
