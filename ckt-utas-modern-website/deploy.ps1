# ckt-utas-modern-website — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/ckt-utas-modern-website/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/ckt-utas-modern-website/",
    [switch]$Build = $false
)
...
$SUBFOLDER     = "ckt-utas-modern-website"
...
Log "INFO" "========================================" Cyan
Log "INFO" "ckt-utas-modern-website DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/ckt-utas-modern-website_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning ckt-utas-modern-website (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set ckt-utas-modern-website
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /ckt-utas-modern-website/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /ckt-utas-modern-website/index.html [QSA,L]
</IfModule>
