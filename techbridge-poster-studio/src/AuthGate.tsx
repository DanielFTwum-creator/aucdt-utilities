...
const AUTH_KEY = 'tuc_auth_techbridge-poster-studio';
const USER_KEY = 'techbridge-poster-studio_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie
      .split('; ')
      .find(row => row.startsWith('techbridge-poster-studio_user='))
      ?.split('=')[1];
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'techbridge-poster-studio_user=; max-age=0; path=/poster/';
      } catch (e) {
        console.error(e);
      }
    }
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/poster/callback`;
