...
const AUTH_KEY = 'tuc_auth_tuc-2026-enrollment-command-centre';
const USER_KEY = 'tuc-2026-enrollment-command-centre_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie
      .split('; ')
      .find(row => row.startsWith('tuc-2026-enrollment-command-centre_user='))
      ?.split('=')[1];
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'tuc-2026-enrollment-command-centre_user=; max-age=0; path=/tuc-2026-enrollment-command-centre/';
      } catch (e) {
        console.error(e);
      }
    }
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/tuc-2026-enrollment-command-centre/callback`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p style={{ fontSize: '12px', color: '#8b90b8', textAlign: 'center', margin: '16px 0 0 0' }}>
