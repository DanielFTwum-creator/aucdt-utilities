# alumni-network - Ultimate Self-Replicating Blueprint (AGENT.md)

> [!IMPORTANT]
> This is an auto-generated monolithic blueprint containing the source code for alumni-network.
...
DATABASE_URL=postgresql://user:password@localhost:5432/alumni_network_db
...
```json
{
  "name": "Alumni Network-backend",
  "version": "1.0.0",
  "description": "Backend API for Alumni Network",
  "main": "dist/server.js",
  "scripts": {
...
```md
# Admin Guide — alumni-network-client

**Application:** alumni-network-client
...
Audit log data is stored in `localStorage` under the key `tuc_alumni-network-client_audit`.
...
```md
# Deployment Guide — alumni-network-client
...
```bash
# From monorepo root
docker-compose -f docker-compose-all-apps.yml build alumni-network-client
```

### Run

```bash
docker-compose -f docker-compose-all-apps.yml up alumni-network-client
...
This Software Requirements Specification (SRS) documents the requirements for **Alumni Network Client**, a web application deployed as part of the Techbridge University College (TUC) institutional utility suite. It serves as the authoritative reference for developers, testers, and stakeholders.

### 1.2 Scope

**Alumni Network Client** is a TypeScript-based React 19 single-page application (SPA) built with Vite and deployed via Docker/Nginx. It operates within the TUC monorepo (`aucdt-utilities`) and conforms to the Techbridge University College Shared Standards.
...
**Alumni Network Client** is a standalone module within the TUC institutional web application suite. It communicates with TUC backend services via REST APIs and shares the TUC design system (Tailwind CSS, Playfair Display, Bebas Neue, Cormorant Garamond).
...
```md
# Testing Guide — alumni-network-client
...
<html lang="en-GB">
  <head>
    ...
    <meta property="og:title" content="Alumni Network" />
    ...
    <meta name="twitter:title" content="Alumni Network" />
...
### FILE: client/src/views/AlumniConnectionsView.tsx
```typescript
...
### FILE: client/src/views/AlumniProfilesView.tsx
```typescript
import React, { useEffect, useState } from 'react';
import {
  AlumniProfile,
...
/**
 * E2E stub — alumni-network-client
 * Extend with Puppeteer/Playwright tests.
 * Run: node scripts/phase3-test-scaffold.js --apply then pnpm test:e2e
 */
describe('alumni-network-client E2E', () => {
...
// Vitest unit test configuration — alumni-network-client
...
// Vitest E2E configuration — alumni-network-client
...
### 1.1 Purpose
This document specifies the functional and non-functional requirements for Alumni Network, a comprehensive management system designed to manage and track Alumni Network operations.

### 1.2 Scope
Alumni Network will provide:
...
### 2.1 Product Perspective
Alumni Network is a web-based management system that integrates with:
...
const AUTH_KEY = 'tuc_auth_alumni_network';
const ACCENT   = '#d97706';
    ...
    <div style={{minHeight:'100vh',background:'#f8fafc',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Inter,system-ui,sans-serif'}}>
      <div style={{background:'#fff',padding:'36px',borderRadius:'16px',boxShadow:'0 4px 24px rgba(0,0,0,0.10)',width:'100%',maxWidth:'420px'}}>
        <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'6px'}}>
          ...
          <h1 style={{fontSize:'20px',fontWeight:'700',color:'#0f172a',margin:0}}>Alumni Network</h1>
        </div>
