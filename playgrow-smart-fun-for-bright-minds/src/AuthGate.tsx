...
const AUTH_KEY = 'tuc_auth_playgrow';
const USER_KEY = 'playgrow_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie.split('; ').find(row => row.startsWith('playgrow_user='))?.split('=')[1];
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'playgrow_user=; max-age=0; path=/playgrow/';
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/playgrow/callback`;
