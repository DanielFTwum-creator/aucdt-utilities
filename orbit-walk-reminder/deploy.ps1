# orbit-walk-reminder — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/orbit-walk-reminder/
...
if ($Build) {
    ...
    $buildDir = "/tmp/orbit-walk-reminder_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning orbit-walk-reminder (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set orbit-walk-reminder
...
if command -v pm2 &>/dev/null; then
  if pm2 describe orbit-walk-reminder &>/dev/null; then
    ...
    cd $RemotePath && PORT=3010 pm2 start server.ts --name orbit-walk-reminder --interpreter npx --interpreter-args tsx --cwd $RemotePath
...
ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -o Serve
