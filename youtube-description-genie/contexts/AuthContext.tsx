...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const initAuth = () => {
      ...
      const cookieValue = document.cookie
        .split('; ')
        .find(row => row.startsWith('youtubegenie_user='))
        ?.split('=')[1];
      if (cookieValue) {
        try {
      ...
      const stored = localStorage.getItem('youtubegenie_user');
      if (stored) {
        try {
  ...
  const login = async (userOrUsername: User | string, password?: string) => {
    if (typeof userOrUsername === 'object') {
      ...
      localStorage.setItem('youtubegenie_user', JSON.stringify(userOrUsername));
      return { success: true };
    }
    ...
    try {
      ...
      if (data.success && data.user) {
        setIsAuthenticated(true);
        setUser(data.user);
        localStorage.setItem('youtubegenie_user', JSON.stringify(data.user));
      }
