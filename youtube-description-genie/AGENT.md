# youtube-description-genie - Ultimate Self-Replicating Blueprint (AGENT.md)

> [!IMPORTANT]
> This is an auto-generated monolithic blueprint containing the source code for youtube-description-genie.
...
const AUTH_KEY = 'tuc_auth_youtube_description_genie';
const ACCENT   = '#db2777';
    ...
    <div style={{minHeight:'100vh',background:'#f8fafc',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Inter,system-ui,sans-serif'}}>
      <div style={{background:'#fff',padding:'36px',borderRadius:'16px',boxShadow:'0 4px 24px rgba(0,0,0,0.10)',width:'100%',maxWidth:'420px'}}>
        <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'6px'}}>
          ...
          <h1 style={{fontSize:'20px',fontWeight:'700',color:'#0f172a',margin:0}}>Youtube Description Genie</h1>
        </div>
...
This application is deployed behind an Nginx reverse proxy at the path `/youtube-description-genie/`.

## Required Configuration for Docker/Nginx Deployment

### 1. Vite Base Path (vite.config.ts)

The Vite config MUST include `base: '/youtube-description-genie/'` to ensure all assets (JS, CSS) load correctly:

```typescript
export default defineConfig(({mode}) => {
  return {
    base: '/youtube-description-genie/',  // REQUIRED: Assets must load from /youtube-description-genie/assets/
    plugins: [react(), ...],
    // ... rest of config
  };
});
...
If using React Router, the BrowserRouter MUST include `basename="/youtube-description-genie"` for client-side routing:

```typescript
createRoot(document.getElementById('root')!).render(
  <StrictMode>
    <BrowserRouter basename="/youtube-description-genie">
      <App />
    </BrowserRouter>
  </StrictMode>,
);
...
- **Nginx Configuration**: The app is served at `http://localhost:8080/youtube-description-genie/`, not at the root
- **Asset Loading**: Without `base: '/youtube-description-genie/'`, assets try to load from `/assets/` instead of `/youtube-description-genie/assets/`
- **Routing**: Without `basename="/youtube-description-genie"`, React Router treats routes incorrectly
...
- Script tags should reference: `/youtube-description-genie/assets/index-*.js`
- Link tags should reference: `/youtube-description-genie/assets/index-*.css`

If they reference `/assets/` instead of `/youtube-description-genie/assets/`, the configuration is incorrect.

## Deployment URLs

- **Development**: `http://localhost:5173` (Vite dev server, no base path needed)
- **Production (Docker)**: `http://localhost:8080/youtube-description-genie/`
- **Production (Staging/Live)**: `https://portal.aucdt.edu.gh/youtube-description-genie/` (or similar)
  ...
  <body class="bg-gray-900">
    ...
    <div id="root">
      <div class="tuc-splash">
        ...
        <div class="tuc-status">youtube description genie</div>
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="w-60 bg-[#0f172a] text-white flex flex-col p-6 shrink-0" aria-label="Admin navigation">
        <div className="flex items-center gap-3 mb-10">
          ...
          <span className="font-bold text-sm">Youtube Description Genie</span>
        </div>
      ...
      <main className="flex-1 p-8 max-w-4xl" role="main">
        <header className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900">Youtube Description Genie — Admin</h1>
