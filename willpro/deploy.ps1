...
param(
    ...
    [string]$RemotePath = "/var/www/vhosts/aucdt.edu.gh/ai-tools.aucdt.edu.gh/willpro/",
    [switch]$Build = $false
)
...
ssh -o StrictHostKeyChecking=no $RemoteHost "chown -R aucdt.edu.gh_md:psacln $RemotePath && chmod -R 755 $RemotePath && chmod 644 $RemotePath/.htaccess 2>/dev/null; true" | Out-Null
