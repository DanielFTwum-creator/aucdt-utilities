...
if ($Build) {
    ...
    $buildDir = "/tmp/willpro_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning willpro (sparse, depth 1)...'
...
timeout 120 pnpm install --no-frozen-lockfile 2>&1 | tail -10 || npm install
log '[4/5] Building...'
...
<IfModule mod_rewrite.c>
  ...
  # Structure mirrors the canonical biochemai .htaccess (do not drift):
  # static files first, then proxy callback/api to the PM2 backend (3015),
  # then SPA fallback. willpro previously collided with groove on 3004.
  ...
  RewriteRule ^api/(.*)$ http://localhost:3015/api/`$1 [P,L]
...
$step6Script = "export NVM_DIR=`"`$HOME/.nvm`"; [ -s `"`$NVM_DIR/nvm.sh`" ] && . `"`$NVM_DIR/nvm.sh`"; nvm use 26 >/dev/null 2>&1 || true; cd $RemotePath && timeout 120 CI=true pnpm install --prod 2>&1 | tail -10 || timeout 120 npm install --omit=dev 2>&1 | tail -10"
...
Log "SUCCESS" "URL  : https://ai-tools.techbridge.edu.gh/willpro/" Green
