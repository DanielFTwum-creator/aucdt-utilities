...
const AUTH_KEY = 'tuc_auth_ai-techbridge';
const USER_KEY = 'ai-techbridge_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie
      .split('; ')
      .find(row => row.startsWith('ai-techbridge_user='))
      ?.split('=')[1];
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'ai-techbridge_user=; max-age=0; path=/ai-techbridge/';
      } catch (e) {
        console.error(e);
      }
    }
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/ai-techbridge/callback`;
