# daaro-distributor — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/sdwater/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/sdwater/",
    [switch]$Build = $false
)
...
Log "INFO" "========================================" Cyan
Log "INFO" "daaro-distributor DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/daaro-distributor_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning daaro-distributor (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set daaro-distributor
...
<IfModule mod_rewrite.c>
  ...
  RewriteRule ^ - [L]
  RewriteRule ^ /sdwater/index.html [QSA,L]
</IfModule>
