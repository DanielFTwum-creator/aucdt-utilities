...
$SUBFOLDER   = 'ai-stand-up-workshop-prep-dashboard'
...
Log -Level 'INFO' -Msg 'WORKSHOP PREP DASHBOARD DEPLOYMENT' -Color Cyan
Log -Level 'INFO' -Msg '========================================' -Color Cyan
...
log '[7/7] Backend deps...'; cp server.ts package.json pnpm-lock.yaml "`$DEPLOY_PATH/" 2>/dev/null || true; cd "`$DEPLOY_PATH" && CI=true pnpm install --prod --silent 2>/dev/null || npm install --omit=dev --silent
log 'Done.'
"@
...
$r=& $SSH @SSH_OPTS $REMOTE "if pm2 describe ${PM2_APP}>\/dev\/null 2>&1; then pm2 reload ${PM2_APP}; echo 'pm2: reloaded'; else cd ${DEPLOY_PATH}; PORT=${PORT} pm2 start server.ts --name ${PM2_APP} --interpreter npx --interpreter-args tsx --cwd ${DEPLOY_PATH}; echo 'pm2: started'; fi"
