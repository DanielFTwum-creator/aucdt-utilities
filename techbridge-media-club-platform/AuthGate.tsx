...
const AUTH_KEY = 'tuc_auth_techbridge-media-club-platform';
const USER_KEY = 'techbridge-media-club-platform_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie.split('; ').find(row => row.startsWith('techbridge-media-club-platform_user='))?.split('=')[1];
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'techbridge-media-club-platform_user=; max-age=0; path=/techbridge-media-club-platform/';
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/techbridge-media-club-platform/callback`;
