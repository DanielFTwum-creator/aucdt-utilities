# techbridge-media-club-platform — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/techbridge-media-club-platform/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/techbridge-media-club-platform/",
    [switch]$Build = $false
)
...
$SUBFOLDER     = "techbridge-media-club-platform"
$PORT          = 3022
$PM2_APP       = "techbridge-media-club-platform"
...
Log "INFO" "========================================" Cyan
Log "INFO" "techbridge-media-club-platform DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/tb-media-club_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning techbridge-media-club-platform (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set techbridge-media-club-platform
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /techbridge-media-club-platform/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /techbridge-media-club-platform/index.html [QSA,L]
</IfModule>
...
Log "SUCCESS" "URL  : https://ai-tools.techbridge.edu.gh/techbridge-media-club-platform/" Green
