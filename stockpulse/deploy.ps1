...
JWT_EXPIRE=7d
GEMINI_API_KEY=AIzaSyCfmVYxWLHxVgOYnq_OOpNKWUr0gqp29yE
...
pm2 save --force
' -replace '{PM2_APP}', $PM2_APP -replace '{REMOTE_PATH}', $RemotePath -replace '{PORT}', $PORT

    & ssh @SSH_OPTS $RemoteHost $pm2Cmd
    ...
    $portCheck = & ssh @SSH_OPTS $RemoteHost "ss -tlnp | grep -q ':$PORT' && echo 'OK' || echo 'WARN'"
    if ($portCheck.Trim() -eq "OK") {
        Write-Host "✅ Port $PORT is listening successfully" -ForegroundColor Green
    } else {
        Write-Host "⚠️ Warning: Port $PORT does not seem to be listening. Check PM2 logs on the server." -ForegroundColor Yellow
    }
