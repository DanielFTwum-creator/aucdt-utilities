...
$SUBFOLDER     = "techbridge-technical-quiz-platform"
$PORT          = 3024
$PM2_APP       = "techbridge-technical-quiz-platform"
...
Log "INFO" "========================================" Cyan
Log "INFO" "techbridge-technical-quiz-platform DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/tb-quiz_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning techbridge-technical-quiz-platform (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set techbridge-technical-quiz-platform
