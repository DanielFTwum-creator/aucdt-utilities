...
const AUTH_KEY = 'tuc_auth_techbridge-technical-quiz-platform';
const USER_KEY = 'techbridge-technical-quiz-platform_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie.split('; ').find(row => row.startsWith('techbridge-technical-quiz-platform_user='))?.split('=')[1];
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'techbridge-technical-quiz-platform_user=; max-age=0; path=/techbridge-technical-quiz-platform/';
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/techbridge-technical-quiz-platform/callback`;
