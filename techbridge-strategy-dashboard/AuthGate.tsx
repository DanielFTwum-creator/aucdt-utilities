...
const AUTH_KEY = 'tuc_auth_techbridge-strategy-dashboard';
const USER_KEY = 'techbridge-strategy-dashboard_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie.split('; ').find(row => row.startsWith('techbridge-strategy-dashboard_user='))?.split('=')[1];
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'techbridge-strategy-dashboard_user=; max-age=0; path=/techbridge-strategy-dashboard/';
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/techbridge-strategy-dashboard/callback`;
