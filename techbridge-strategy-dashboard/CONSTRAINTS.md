# CONSTRAINTS.md — Techbridge Strategy Dashboard

> Environment specification for the Techbridge Strategy Dashboard app.
...
| Field | Value |
|---|---|
| App name | Techbridge Strategy Dashboard |
| PM2 process | `techbridge-strategy-dashboard` (proposed; not yet deployed) |
| Port | **3037** (reserved in SERVER_PORTS.md, 1 Jul 2026; not yet binding) |
| Public URL | `https://ai-tools.techbridge.edu.gh/techbridge-strategy-dashboard/` (planned) |
| Deploy path | `/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/techbridge-strategy-dashboard/` (planned) |
| Stack | React + Vite (frontend) · Express + tsx (backend) · Node v26 |
| Status | **Not yet deployed** — port reserved, deploy.ps1 and server.ts defaults already set to 3037 |
...
| `PORT` | Express listen port | Must be `3037` — matches the SERVER_PORTS.md reservation |
...
```powershell
cd C:\Development\github\aucdt-utilities\techbridge-strategy-dashboard
.\deploy.ps1 -Build
```

At first deploy, in order:
1. Add the nginx vhost route for `/techbridge-strategy-dashboard/` (SPA + `/api/` proxy to 3037)
...
☐ Health check passes: GET /techbridge-strategy-dashboard/api/health -> { ok: true }
☐ PORT=3037 confirmed against SERVER_PORTS.md before pm2 start
```
