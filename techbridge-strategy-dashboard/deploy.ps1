# techbridge-strategy-dashboard — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/techbridge-strategy-dashboard/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/techbridge-strategy-dashboard/",
    [switch]$Build = $false
)
...
Log "INFO" "========================================" Cyan
Log "INFO" "techbridge-strategy-dashboard DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/tb-strategy_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning techbridge-strategy-dashboard (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set techbridge-strategy-dashboard
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /techbridge-strategy-dashboard/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /techbridge-strategy-dashboard/index.html [QSA,L]
</IfModule>
...
Log "SUCCESS" "URL  : https://ai-tools.techbridge.edu.gh/techbridge-strategy-dashboard/" Green
