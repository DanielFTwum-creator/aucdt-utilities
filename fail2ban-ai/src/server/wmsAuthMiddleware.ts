...
/**
 * WMS SSO guard for the fail2ban-ai API (TUC-ICT-SRS-2026-013, staff-only).
 * Requires a valid WMS access token (Bearer) on every protected request — the
 * frontend obtains it via the WMS SSO flow. Trust is established by relaying the
 * token to WMS /api/me (no shared secret); a short in-memory cache avoids
 * hammering WMS. Domain-gated to @techbridge.edu.gh.
 *
 * This closes the exposure of the banned-IP / threat-intelligence API, which
 * reveals the server's own defensive posture.
 */
...
export async function requireWmsAuth(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    if (!email.endsWith(ALLOWED_DOMAIN)) {
      res.status(403).json({ error: 'Forbidden — Techbridge staff only.' });
      return;
    }
