# teamwork-accountability-diagnostic — Deploy Script (static SPA, no backend)
# URL: https://ai-tools.techbridge.edu.gh/teamwork/
# Usage: .\deploy.ps1 -Build
#
# Pure static SPA: no server.ts, no PM2, no port. Apache serves dist/ from the
# web root with an .htaccess SPA fallback. Nothing to relay, no auth.

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/teamwork/",
    [switch]$Build = $false
)
...
$SUBFOLDER     = "teamwork-accountability-diagnostic"
...
Log "INFO" "========================================" Cyan
Log "INFO" "teamwork-accountability-diagnostic DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
Log "SUCCESS" "Pre-flight OK (static SPA — no secrets, no backend)" Green
...
if ($Build) {
    ...
    $buildDir = "/tmp/teamwork_deploy_$commit"
    $serverScript = @"
set -e
...
"@
    ...
    else { Log "ERROR" "Server build failed (exit $LASTEXITCODE) — aborting before .htaccess/permissions. Fix the build, do not treat this as deployed." Red; exit 1 }
    ...
    if (-not (Select-String -Path "dist/index.html" -Pattern '<script[^>]+(src="[^"]*\.js"|type="module")' -Quiet)) { Log "ERROR" "dist/index.html ships no JS bundle. Aborting deploy." Red; exit 1 }
...
Log "INFO" "Step 4: Writing .htaccess (SPA fallback)..." Yellow
@"
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /teamwork/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /teamwork/index.html [QSA,L]
</IfModule>
...
Log "SUCCESS" "URL  : https://ai-tools.techbridge.edu.gh/teamwork/" Green
