...
const USER_KEY = 'techbridge_student_population_register_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie.split('; ').find(row => row.startsWith('techbridge_student_population_register_user='))?.split('=')[1];
    if (cookieValue) { try { const userData = JSON.parse(atob(decodeURIComponent(cookieValue))) as User; localStorage.setItem(USER_KEY, JSON.stringify(userData)); sessionStorage.setItem(AUTH_KEY, '1'); setUser(userData); setAuthed(true); document.cookie = 'techbridge_student_population_register_user=; max-age=0; path=/techbridge-student-population-register/'; } catch (e) { console.error('Failed to parse user cookie:', e); } }
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/techbridge-student-population-register/callback`;
