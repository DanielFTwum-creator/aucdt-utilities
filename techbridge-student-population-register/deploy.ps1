# techbridge-student-population-register — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/techbridge-student-population-register/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/techbridge-student-population-register/",
    [switch]$Build = $false
)
...
$SUBFOLDER     = "techbridge-student-population-register"
...
Log "INFO" "========================================" Cyan
Log "INFO" "techbridge-student-population-register DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/tb-student-reg_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning techbridge-student-population-register (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set techbridge-student-population-register
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /techbridge-student-population-register/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /techbridge-student-population-register/index.html [QSA,L]
</IfModule>
...
if command -v pm2 &>/dev/null; then
  if pm2 describe tb-student-reg &>/dev/null; then
    ...
    cd $RemotePath && PORT=3000 pm2 start server.ts --name tb-student-reg --interpreter npx --interpreter-args tsx
...
Log "SUCCESS" "URL  : https://ai-tools.techbridge.edu.gh/techbridge-student-population-register/" Green
