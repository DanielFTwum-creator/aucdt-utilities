// WMS SSO client for Patois Lyricist (archetype B — TUC-ICT-SRS-2026-013).
// Sign-in is delegated to the WMS backend (Google OAuth + TOTP), domain-gated to
// @techbridge.edu.gh so all TUC members (students + staff) can use the lyricist.
// This service handles the WMS dance only: code exchange, MFA, and silent session
// adoption via the shared .techbridge.edu.gh refresh cookie (single sign-on across
// the fleet). The access token it returns is injected into this app's /patois/api/
// calls by the fetch wrapper in contexts/AuthContext.tsx.

const WMS_BASE = (import.meta as any).env?.VITE_WMS_BASE ?? 'https://wms.techbridge.edu.gh';

export const wmsAuthBase: string = WMS_BASE;
...
// The wrapped window.fetch (see AuthContext) only touches same-origin /patois/api/
// calls; these cross-origin WMS calls go through the browser's native fetch and
// carry the fleet refresh cookie via credentials: 'include'.
