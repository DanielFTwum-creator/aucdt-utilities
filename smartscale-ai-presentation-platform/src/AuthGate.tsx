...
const AUTH_KEY = 'tuc_auth_smartscale-ai-presentation-platform';
const USER_KEY = 'smartscale-ai-presentation-platform_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie.split('; ').find(row => row.startsWith('smartscale-ai-presentation-platform_user='))?.split('=')[1];
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'smartscale-ai-presentation-platform_user=; max-age=0; path=/smartscale-ai-presentation-platform/';
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/smartscale-ai-presentation-platform/callback`;
