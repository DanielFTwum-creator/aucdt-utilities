# techbridge-promo - Ultimate Self-Replicating Blueprint (AGENT.md)

> [!IMPORTANT]
> This is an auto-generated monolithic blueprint containing the source code for techbridge-promo.
...
This application is deployed behind an Nginx reverse proxy at the path `/techbridge-promo/`.

## Required Configuration for Docker/Nginx Deployment

### 1. Vite Base Path (vite.config.ts)

The Vite config MUST include `base: '/techbridge-promo/'` to ensure all assets (JS, CSS) load correctly:

```typescript
export default defineConfig(({mode}) => {
  return {
    base: '/techbridge-promo/',  // REQUIRED: Assets must load from /techbridge-promo/assets/
    plugins: [react(), ...],
    // ... rest of config
  };
});
...
If using React Router, the BrowserRouter MUST include `basename="/techbridge-promo"` for client-side routing:

```typescript
createRoot(document.getElementById('root')!).render(
  <StrictMode>
    <BrowserRouter basename="/techbridge-promo">
      <App />
    </BrowserRouter>
  </StrictMode>,
);
...
- **Nginx Configuration**: The app is served at `http://localhost:8080/techbridge-promo/`, not at the root
- **Asset Loading**: Without `base: '/techbridge-promo/'`, assets try to load from `/assets/` instead of `/techbridge-promo/assets/`
- **Routing**: Without `basename="/techbridge-promo"`, React Router treats routes incorrectly
...
- Script tags should reference: `/techbridge-promo/assets/index-*.js`
- Link tags should reference: `/techbridge-promo/assets/index-*.css`

If they reference `/assets/` instead of `/techbridge-promo/assets/`, the configuration is incorrect.

## Deployment URLs

- **Development**: `http://localhost:5173` (Vite dev server, no base path needed)
- **Production (Docker)**: `http://localhost:8080/techbridge-promo/`
- **Production (Staging/Live)**: `https://portal.aucdt.edu.gh/techbridge-promo/` (or similar)
...
```md
# Admin Guide — techbridge-promo

**Application:** techbridge-promo
...
Audit log data is stored in `localStorage` under the key `tuc_techbridge-promo_audit`.
...
```md
# Deployment Guide — techbridge-promo
...
```bash
# From monorepo root
docker-compose -f docker-compose-all-apps.yml build techbridge-promo
```

### Run

```bash
docker-compose -f docker-compose-all-apps.yml up techbridge-promo
...
This Software Requirements Specification (SRS) documents the requirements for **Techbridge Promo**, a web application deployed as part of the Techbridge University College (TUC) institutional utility suite. It serves as the authoritative reference for developers, testers, and stakeholders.

### 1.2 Scope

**Techbridge Promo** is a TypeScript-based React 19 single-page application (SPA) built with Vite and deployed via Docker/Nginx. It operates within the TUC monorepo (`aucdt-utilities`) and conforms to the Techbridge University College Shared Standards.
...
**Techbridge Promo** is a standalone module within the TUC institutional web application suite. It communicates with TUC backend services via REST APIs and shares the TUC design system (Tailwind CSS, Playfair Display, Bebas Neue, Cormorant Garamond).
...
```md
# Testing Guide — techbridge-promo
...
<html lang="en-GB">
  <head>
    ...
    <meta property="og:title" content="Techbridge Promo | Techbridge University College" />
    ...
    <meta name="twitter:title" content="Techbridge Promo | Techbridge University College" />
    ...
    <title>Techbridge Promo | Techbridge University College</title>
  ...
  <body>
    ...
    <div id="root">
      <div class="tuc-splash">
        ...
        <div class="tuc-status">techbridge promo</div>
...
const AUTH_KEY = 'tuc_auth_techbridge_promo';
const ACCENT   = '#ea580c';
    ...
    <div style={{minHeight:'100vh',background:'#f8fafc',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Inter,system-ui,sans-serif'}}>
      <div style={{background:'#fff',padding:'36px',borderRadius:'16px',boxShadow:'0 4px 24px rgba(0,0,0,0.10)',width:'100%',maxWidth:'420px'}}>
        <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'6px'}}>
          ...
          <h1 style={{fontSize:'20px',fontWeight:'700',color:'#0f172a',margin:0}}>Techbridge Promo</h1>
        </div>
...
### FILE: src/components/ProgramGrid.tsx
```typescript
import { motion } from "framer-motion";
import React from "react";
...
### FILE: src/components/PromotionalFlyer.tsx
```typescript
import { motion } from "framer-motion";
import React from "react";
...
### FILE: src/components/StatsBoard.tsx
```typescript
...
### FILE: src/index.css
```css
@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;900&family=Rajdhani:wght@400;500;600;700&family=Cinzel:wght@700&display=swap');
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside className="w-60 bg-[#0f172a] text-white flex flex-col p-6 shrink-0" aria-label="Admin navigation">
        <div className="flex items-center gap-3 mb-10">
          ...
          <span className="font-bold text-sm">Techbridge Promo</span>
        </div>
      ...
      <main className="flex-1 p-8 max-w-4xl" role="main">
        <header className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900">Techbridge Promo — Admin</h1>
...
/**
 * E2E stub — techbridge-promo
 * Extend with Puppeteer/Playwright tests.
 * Run: node scripts/phase3-test-scaffold.js --apply then pnpm test:e2e
 */
describe('techbridge-promo E2E', () => {
...
// Vitest unit test configuration — techbridge-promo
...
// Vitest E2E configuration — techbridge-promo
