# markai — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/markai/
...
Log "INFO" "========================================" Cyan
Log "INFO" "markai DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    if (Test-Path ".env.local") {
        scp -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -o ServerAliveCountMax=3 ".env.local" "${RemoteHost}:/tmp/.env.markai.build" 2>$null | Out-Null
    }
...
log '[2/5] Cloning markai (sparse, depth 1)...'
...
"@
    ...
    ssh -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -o ServerAliveCountMax=3 $RemoteHost "rm -rf $buildDir; rm -f /tmp/.env.markai.build" 2>$null | Out-Null
} else {
...
scp -o StrictHostKeyChecking=no -o ServerAliveInterval=30 -o ServerAliveCountMax=3 server.cjs package.json pnpm-lock.yaml "${RemoteHost}:${RemotePath}" 2>$null | Out-Null
if (Test-Path ".env.local") { 
