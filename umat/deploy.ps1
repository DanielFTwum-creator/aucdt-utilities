# umat — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/umat/
...
if ($Build) {
    ...
    $buildDir = "/tmp/umat_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning umat (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set umat
cd umat
...
Log "SUCCESS" "URL  : https://ai-tools.techbridge.edu.gh/umat/" Green
