# techbridge-ai-workshop-flyer — Deploy Script
...
$SUBFOLDER     = "techbridge-ai-workshop-flyer"
...
Log "INFO" "========================================" Cyan
Log "INFO" "techbridge-ai-workshop-flyer DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/tb-workshop-flyer_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning techbridge-ai-workshop-flyer (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set techbridge-ai-workshop-flyer
...
Log "SUCCESS" "URL  : https://ai-tools.techbridge.edu.gh/techbridge-ai-workshop-flyer/" Green
