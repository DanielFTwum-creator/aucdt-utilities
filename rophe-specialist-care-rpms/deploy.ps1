# rophe-specialist-care-rpms — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/rophe-specialist-care-rpms/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/rophe-specialist-care-rpms/",
    [switch]$Build = $false
)
...
$SUBFOLDER     = "rophe-specialist-care-rpms"
...
Log "INFO" "========================================" Cyan
Log "INFO" "rophe-specialist-care-rpms DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/rophe_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning rophe-specialist-care-rpms (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set rophe-specialist-care-rpms
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /rophe-specialist-care-rpms/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /rophe-specialist-care-rpms/index.html [QSA,L]
</IfModule>
