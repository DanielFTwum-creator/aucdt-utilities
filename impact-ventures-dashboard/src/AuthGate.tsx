...
const AUTH_KEY = 'tuc_auth_impact_ventures_dashboard';
const USER_KEY = 'impact_ventures_dashboard_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie.split('; ').find(row => row.startsWith('impact_ventures_dashboard_user='))?.split('=')[1];
    if (cookieValue) { try { const userData = JSON.parse(atob(decodeURIComponent(cookieValue))) as User; localStorage.setItem(USER_KEY, JSON.stringify(userData)); sessionStorage.setItem(AUTH_KEY, '1'); setUser(userData); setAuthed(true); document.cookie = 'impact_ventures_dashboard_user=; max-age=0; path=/impact-ventures-dashboard/'; } catch (e) { console.error('Failed to parse user cookie:', e); } }
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri = import.meta.env.VITE_GOOGLE_REDIRECT_URI || `${window.location.origin}/impact-ventures-dashboard/callback`;
