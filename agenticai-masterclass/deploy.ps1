# Agenticai Masterclass Deployment Script
# Simple SCP-based deployment

param(
    [string]$RemoteHost = "root@66.226.72.199",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/agenticai-masterclass/",
    [switch]$Build = $false
)

Write-Host "=== AGENTICAI MASTERCLASS DEPLOYMENT ===" -ForegroundColor Cyan
...
bash -c "cd 'C:\Development\github\aucdt-utilities\agenticai-masterclass' && scp -r -o StrictHostKeyChecking=no dist/* $RemoteHost`:$RemotePath 2>/dev/null"

Write-Host "Creating .htaccess..." -ForegroundColor Yellow
@"
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /agenticai-masterclass/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /agenticai-masterclass/index.html [QSA,L]
</IfModule>
...
Write-Host "URL: https://ai-tools.techbridge.edu.gh/agenticai-masterclass`n"
