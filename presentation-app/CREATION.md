...
1. **Email Templates** — three ready-to-send long-form copy decks: *Cold Outreach*, *Warm Introduction Follow-up*, and *Event/Conference Follow-up*. Each shows a subject line and full body in a monospaced `<pre>` block; the partnership lead pastes them into Outlook / Gmail and edits the bracketed placeholders.
2. **Pitch Presentation** — a slide deck titled **"Designing Africa's Future"** (Mastercard Foundation × TUC). Slides include hero copy, statistical callouts ("60% youth unemployed", "$23B digital opportunity by 2030", "2% global designers from Africa", "58% financial inclusion vs 76% global"), and impact-alignment grids.

The two views are toggled via a top-tab navigation (`emails` | `presentation`). Slides are advanced via Prev/Next buttons or pagination dots; the hash navigates linearly through `slides.length` (currently 3 slides — extensible).
...
| Icons | **lucide-react** ^0.525.0 (newer than other apps in the repo) — used for ChevronLeft/Right, Mail, FileText, Users, Target, Building, Lightbulb, TrendingUp, Award, Calendar, CheckCircle, Phone |
...
├── index.html
├── index.css
├── package.json                # name: presentation-app, version: 0.1.0
├── pnpm-lock.yaml
├── pnpm-workspace.yaml
├── vite.config.ts              # dev port 3000, base './', tailwindcss plugin
├── vitest.config.ts
...
└── src/
    ...
    ├── index.css
    ├── App.tsx                 # presentation root (~320 LOC)
    ├── App.css
    ├── App.test.js
    ...
    ├── setupTests.js
    ├── logo.svg
    ├── postcss.config.tsx      # repo-tracking variant; functional config is at root
    ├── tailwind.config.tsx     # repo-tracking variant; functional config is at root
    ├── vite-env.d.ts
    ├── __tests__/
    ├── components/
    │   └── ProtectedRoute.tsx
...
- `slides: { title: string; subtitle: string; content: ReactNode }[]` — currently 3 entries (see §7).
- `emailTemplates: { title: string; subject: string; content: string }[]` — currently 3 entries (see §6).
...
- 2-column grid:
  - Blue card `bg-blue-50` — `<Building />` icon, `Techbridge University College`, "Ghana's Premier Design & Technology Institution".
  - Green card `bg-green-50` — `<Target />` icon, `Mastercard Foundation`, "Empowering African Youth & Innovation".

### Slide 2 layout — 2×2 stat grid
...
### Slide 3 layout — alignment list

Four pill rows on tinted backgrounds:

1. `bg-blue-50` + `<Target />` → **Financial Inclusion** → Training designers who create accessible financial products.
2. `bg-green-50` + `<Users />` → **Youth Employment** → Direct job creation + entrepreneurship pipeline.
3. `bg-purple-50` + `<Award />` → **African Leadership** → Building local capacity for indigenous solutions.
4. `bg-orange-50` + `<TrendingUp />` → **Systems Change** → Transforming how technology serves African communities.
...
- **Top tab bar** — `bg-white shadow-sm border-b`. Tabs: `Email Templates` (Mail icon), `Pitch Presentation` (FileText icon). Active tab: `border-b-2 border-blue-500 text-blue-600`.
- **Content container** — `max-w-6xl mx-auto p-6`.
- **Email view** — heading `Email Templates`, subtitle `Ready-to-use outreach templates for Mastercard Foundation`, then a vertical stack of three template cards.
- **Presentation view** — header card showing `slides[currentSlide].title`, `slides[currentSlide].subtitle`, slide counter `Slide X of Y`, and Prev/Next chevron buttons. Below: slide content card `bg-white rounded-lg shadow-sm border p-8 min-h-96`. Below that: pagination dots — `w-3 h-3 rounded-full`, active `bg-blue-500`, inactive `bg-gray-300`.
...
- Login card title: `"Presentation App"`. Footer: `"Techbridge University College · admin / admin"`.
...
- **Main pane** header `Presentation App — Admin · Techbridge University College · Staff Portal`.
...
- **nginx.conf** — `listen 80`, root `/usr/share/nginx/html`, `try_files $uri $uri/ /index.html`, `/health` returns `healthy`, security headers + gzip + 1-year immutable static cache.
- **docker-compose-all-apps.yml** — service `presentation-app`, context `./presentation-app`, dockerfile `../Dockerfile.vite`, network `tuc-network`, healthcheck against `http://localhost/health`.
...
- Colour contrast: header gradients must keep `text-white` on `from-orange-500 to-red-600` (≥ 4.5:1).
...
| AC-4 | Emails view renders exactly 3 templates with the subjects in §6 |
| AC-5 | Presentation view renders exactly 3 slides with the titles/subtitles in §7 |
...
| AC-9 | Admin Overview shows the six compliance tiles in §10 |
...
| AC-12 | All NEW slide / email copy refers to **Techbridge University College** / **TUC** — never AUCDT |
