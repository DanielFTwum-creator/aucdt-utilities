# An-Elephant-on-Parade — Deploy Script
# URL: https://ai-tools.techbridge.edu.gh/elephant/
# Usage: .\deploy.ps1 -Build

param(
    [string]$RemoteHost = "root@techbridge.edu.gh",
    [string]$RemotePath = "/var/www/vhosts/techbridge.edu.gh/ai-tools.techbridge.edu.gh/elephant/",
    [switch]$Build = $false
)
...
$SUBFOLDER     = "An-Elephant-on-Parade"
...
Log "INFO" "========================================" Cyan
Log "INFO" "An-Elephant-on-Parade DEPLOYMENT" Cyan
Log "INFO" "========================================" Cyan
...
if ($Build) {
    ...
    $buildDir = "/tmp/an-elephant-on-parade_deploy_$commit"
    $serverScript = @"
set -e
...
log '[2/5] Cloning An-Elephant-on-Parade (sparse, depth 1)...'
git clone --depth 1 --filter=blob:none --sparse '$GITHUB_REPO' $buildDir
cd $buildDir
git sparse-checkout set An-Elephant-on-Parade
cd An-Elephant-on-Parade
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /elephant/
  RewriteCond %{REQUEST_FILENAME} -f [OR]
  RewriteCond %{REQUEST_FILENAME} -d
  RewriteRule ^ - [L]
  RewriteRule ^ /elephant/index.html [QSA,L]
</IfModule>
