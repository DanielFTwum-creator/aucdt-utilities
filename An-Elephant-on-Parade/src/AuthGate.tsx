...
const AUTH_KEY = 'tuc_auth_elephant';
const USER_KEY = 'elephant_user';
...
export function AuthGate({ children, onLogout }: { children: React.ReactNode; onLogout?: () => void }) {
  ...
  useEffect(() => {
    const cookieValue = document.cookie
      .split('; ')
      .find(row => row.startsWith('elephant_user='))
      ?.split('=')[1];
    ...
    if (cookieValue) {
      try {
        ...
        setUser(userData);
        setAuthed(true);
        document.cookie = 'elephant_user=; max-age=0; path=/elephant/';
      } catch (e) {
        console.error(e);
      }
    }
  ...
  const handleGoogleLogin = () => {
    ...
    const redirectUri =
      ...
      `${window.location.origin}/elephant/callback`;
